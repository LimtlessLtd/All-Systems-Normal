...
public static class NpcPromptBuilder
{
    ...
    public static string Build(Npc npc, GameState state)
    {
        ...
        builder.AppendLine("For OpenDoor/CloseDoor/LockDoor/UnlockDoor/ReadAccessLog, TargetId must be an exact adjacent hatch ID. Lock/unlock is only valid when your role/skills grant authority. ReadAccessLog needs the hatch powered; a keycard entry names the card's owner, not necessarily who used it.");
