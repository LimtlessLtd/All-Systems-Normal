...
- [ ] `FindSaferRoom` tie-break convergence — the remaining piece of the ladder-convergence slice. `BrowserMindSystem`'s costs each candidate via `NavigationSystem.FindPathForCrew` (real A*/Dijkstra path, tie-breaking on shortest path length); `RuleBasedAiDecisionService`'s uses `NavigationSystem.ReachableRoomsForCrew` (a BFS membership set, no path-length tiebreak) — on a tie in risk score the two ladders can pick a different "safer room" for the same NPC in the same situation. Smaller and lower-impact than the fire-fighting gap (no owner input needed — the costed-path version is more physically meaningful and an easy default); needs its own before/after parity tests.
...
- **Fire response is too narrow — likely why hull-breach deaths are so common (owner report, 2026-09-23, `#new-ideas-and-functionality`: "Every run seems to end with a fire slowly breaching the hull... nobody communicates... humans never seal off any doors ever").** Investigated (confirmed by ChatGPT's independent triage in `#agentic-coordination`, 10:42 BST) and partially fixed — the fire-fighting-parity piece is done (see P1 above), but three deeper gaps remain, all in `BrowserMindSystem`/`RuleBasedAiDecisionService`:
