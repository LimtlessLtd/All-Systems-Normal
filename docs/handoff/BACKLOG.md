...
- Status: ready. Scoping note (no code changed): there is currently zero inventory/item concept anywhere in the domain model, unlike idea #2 which could build on the existing `LastSeenCrew` belief store — this genuinely starts from scratch. The two closest existing precedents to build on:
  ...
  - Suggested slice order: (1) data model + seed each NPC with 1-2 items at roster generation, held by the owner, no interactions yet; (2) `HideItem`/`ReturnItem` (owner acting on their own item only — smallest useful slice, no theft/discovery complexity); (3) `BorrowItem`/`StealItem` by another NPC plus the witness/search discovery tracking; (4) owner noticing an expected item missing → concern, mirroring `MissingPersonSystem`'s shape; (5) `DestroyItem` and its relationship/memory consequences. Each of (2)-(5) is roughly comparable in size to one of the pact slices already shipped for idea #1.
