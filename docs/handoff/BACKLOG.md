...
- Status: **in progress**. Slice 1 is shipped: fire is camera knowledge (`OverseerSightSystem`, see `SYSTEMS.md` → infrastructure). A blind room shows its last camera-seen fire and when it was seen, never live flames, and the ALERTS list now reads the #22 reported O₂/temperature channels. Slice 2 is shipped: the event log's fire lines are camera knowledge too (`OverseerSightSystem.Witnessed` tags a blind room's line `[UNSEEN]`, hidden from the player's station log but kept for debug telemetry), and a fire-caused hull failure in a blind room shows only a pressure-sensor `HULL BREACH` line. Remaining: (1) other camera-only facts still read live truth in a blind room: blood/bodies (partly gated), the Inspector's robot/turret panels, the room callout's and Inspector's hull state (a breach is arguably pressure-sensor knowledge, but the integrity % is not), and crew-action log lines written in a blind room (e.g. `FightFire`/`VentHazardRoom` results, attacks, thefts), which can route through the same `Witnessed` helper. The right place is `CrewTaskSystem.Log` (every task's start/complete/fail line, e.g. `FightFire`'s "knocks the fire down to 40%" is written there as well as in `StationHazardSystem`), but hiding what crew do in a blind room hinges on the owner question in (2), so it waits on that answer; the ALERTS list was reviewed and every entry is sensor, network or biotelemetry knowledge, not camera knowledge; (2) crew location in a blind room is still "telemetry only" in the Inspector, a deliberate choice worth confirming with the owner before hiding; (3) a microphone/audio channel and control-bus loss as further blind-spot sources (pairs with #21's section partitions); (4) last-seen presentation for crew (a ghost token at the last seen position).
...
- Status: **in progress**. Slices 1–2 are shipped (see `SYSTEMS.md` → local/manual control mode): a per-machine `IsLocalControl` flag, the crew switches both ways at the hardware, and Overseer's remote machine and life-support toggles are refused while it is set. Remaining: (1) doors: a hatch already has its own crew `OverrideDoor` / `IsManuallyOverridden` path, so decide whether a local-control switch on a door console should reuse it rather than add a second flag; (2) crop beds and the other Overseer verbs that act on machinery indirectly (e.g. room power) if the owner wants those gated too; (3) the fallback minds never switch, so Pages never shows it. The map marker shipped in slice 2 (a dashed amber fixture outline). Natural pairing with #21.
...
- Status: **in progress**. Slice 1 is shipped (see `SYSTEMS.md` → physical credentials): a hatch-lock keycard possession is seeded for crew with role-trained lock authority, and whoever holds one can lock and unlock hatches, so lending and theft already work through the possession affordances. Remaining: (1) per-door or per-area scopes (e.g. a Security keycard that opens only Security's hatches), so that "giving one engineer access to Security" is a specific grant; (2) an Overseer grant/revoke verb (issue, cancel or re-scope a card), which pairs with #26's access log; (3) PIN knowledge as a shareable memory and biometric access as a non-transferable credential. Deciding whether role alone should stop granting authority (so that a stolen card really locks its owner out) is an owner call to raise before that slice; (4) console and machine access as well as hatches.
...
- Status: **in progress**. Slice 1 is shipped (see `SYSTEMS.md` → forensic access logs): every hatch keeps a bounded access log of locks, unlocks, forced opens and bypasses, with the credential and the identity the controller could actually know (a keycard entry names the card's owner, so a stolen card frames them). The door Inspector shows it. Slice 2 is shipped: crew can `ReadAccessLog` at an adjacent powered hatch, remembering what the controller recorded. Slice 3 is shipped: a technically skilled crew member can `WipeAccessLog`, which leaves a `LOG WIPED` trace and a sensitive (#14) memory for the wiper and any witness. Remaining: (1) the fallback minds never read or wipe a log, so Pages never shows either; (2) falsifying an entry (rewrite `RecordedName`/`RecordedId` to someone else while `ActorId` keeps the truth). This needs a second target, the name to frame, which the one-target action shape can't express yet, so it probably waits on #5's plan steps or a `door:person` target convention; (3) consoles, airlocks and machinery as well as hatches (pairs with #25 (4)); (4) Overseer reads the log over the control bus, so a partitioned network (#21) should hide it, as it does not yet.
...
- Status: **in progress**. Slice 1 is shipped (see `SYSTEMS.md` → station policies). Overseer can enact or lift six policies (rationing, curfew, weapons prohibition and mandatory medical checks station-wide; quarantine and restricted area for a room). Each is announced over the #27 intercom, crew remember the ones they heard (so someone who missed a lift still believes the rule stands), and a prompt block lists them as Overseer's rules. Remaining: (1) deterministic enforcement where physical: a quarantine or restricted area could lock or flag its hatches, rationing could cap meals drawn per person, and the weapons ban could make a carried weapon visible evidence of a breach; (2) consequences a mind can compose, e.g. witnessing a breach becomes a memory (#16) and crew can ReportConcern about it; (3) the fallback minds ignore policies, so Pages shows only the announcement; (4) persisting policies across a campaign, if the owner wants that.
...
- Status: **in progress**. Slice 1 is shipped (see `SYSTEMS.md` → recreation activities): a wall-mounted Television in the lounge, and `Recreate` activities `watch-tv`, `play-games` and `read`, each tied to its fixture with its own consequences. Shared TV watching eases the social need, and a dead TV or console gives no break. Slice 2 is shipped: `play-cards` at the lounge table, which needs a second player (see `SYSTEMS.md` → recreation activities). The fallback minds rarely play it: a browser-mind soak (8 seeds × 24h) had 1 card-playing minute, because two lonely people (social ≥ 45) seldom meet in the lounge, where mean social need is ~28. It is mainly a model-chosen activity. Loosen the fallback pick only if the owner wants cards visible on Pages. The "more solo activities" slice is partly shipped: `write` at the Crew Quarters writing desk, the first activity outside the lounge (each activity now names its room). It is model-chosen only, because the fallback pick is lounge-only, so Pages never shows it. The desk chair is also a carried-meal seat in the quarters (#90), and nothing stops an eater and a writer from both using it, the same pre-existing overlap as the lounge's reading chair. Remaining: (1) a galley radio or another room's activity (needs a new fixture and its art); (2) should a fallback mind ever write, e.g. a stressed person already in the quarters? That is an owner call, like cards; (3) crew-token poses/animation for the activity, alongside the open "activity animations" aesthetics item.
...
- Also from the owner (#105, 2026-09-24): "in order to get to a releasable version we should have most of the rimworld functionality implemented too". Treat RimWorld as the release bar for simulation breadth and readability. Don't copy its assets or mechanics literally, and keep each concrete gap as its own backlog item under the core LLM/deterministic-C# rule.
