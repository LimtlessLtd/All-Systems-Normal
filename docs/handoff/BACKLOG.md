...
- **Grid shed order vs its own contract (soak finding, 2026-09-24).** `StationUpkeepSystem.ShedLoad`'s comment says "Life support, the reactor and the corridor are the last things to go", but `Priority` sheds Engineering (O₂ generator, scrubbers, life-support core) at 6, before Medical (7), Control (8) and every other room type (9). In the seed-10 soak, generation fell short by about 40 kW, and Engineering went dark while the Control Room stayed lit. Crew servicing the worn reactor now recovers it within a few hours (`RestorableOutageTests`), but suffocation margins would be wider if Engineering were shed last. That's a small change to `Priority`, but it changes which rooms go dark first in every brownout, so run a before/after soak. Related soak note: Secure Continuity ends at T+08:00 (`ObservationWindow`), so a "24h" soak of it only covers 8h. Long-run #80 soaks need a scenario or harness that keeps running.
