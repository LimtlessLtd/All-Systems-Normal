...
- Status: **in progress**. Slice 1 is shipped: chairs, sofas and tables. Tables have a raised top with a lip and shadow, warm wood in the galley/lounge and light grey elsewhere. Seats have a cushion and a backrest on the far side from what they face (`SeatFacingRules`: sofas face a TV/screen, chairs the nearest table/console/workbench, falling back to the seed's facing). Sofas are split into cushions. Slice 2 is shipped: the television is a flat panel with a thick bezel, a picture with glare, a media-stand edge and a standby LED, and it goes dark with a red LED when unpowered. The game console (`RecreationConsole`) is an arcade cabinet (neon trim, pixel-game screen, joystick, two buttons) instead of a copy of the work console. A sofa faces the television before any nearer wall `Screen`. Slice 3 is shipped: the galley line is a steel counter with a 2x2 hob in one corner (its rings glow while the counter runs and go cold when it's inert) and a cutting board at the far end. Every sink is a steel rim around a recessed basin with a tap and a drain. The galley's stores cabinets are a cold pantry with split doors, handles and a blue temperature light. Cross-room pass, first part: showers (a tiled tray with a drain, a head and a glass edge), toilets (a bowl and a cistern) and mirrors (framed glass with a glint) no longer use the generic grey block. Beds now have a frame, a pillow at the end where the sleeper's head lies (left, or top for portrait beds; one `BedIsLandscape` helper drives both) and a blanket in the room accent. Benches are wooden slats, and crates are braced cargo boxes. Remaining: no furniture falls back to the generic block any more. Windows and some machines (utility panels, door consoles, life-support units) are still generic or drawn only through their system class. Whether they need a pass is a judgement call for a later playtest, not an open slice. The generated kitchens scatter chairs rather than setting them around the table. That's a generation layout issue, separate from art.
