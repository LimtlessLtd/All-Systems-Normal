...
- Status: **in progress**. Slice 1 shipped: actions with no valid target for this person right now are left out of the capability list, together with their target contracts (~33.6k → ~30.4k characters on the default station; see `SYSTEMS.md` → Ollama decision budget). Next: status/topology to local + known-problem rooms, then preamble tightening. Bullet 1 of the same message (decision output cap 300 → 750) was pushed by the owner directly; the `num_ctx` stop-gap and its budget test shipped alongside.
