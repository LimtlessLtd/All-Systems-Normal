...
- Status: **in progress**. Slice 1 is shipped (see `SYSTEMS.md` → forensic access logs): every hatch keeps a bounded access log of locks, unlocks, forced opens and bypasses, with the credential and the identity the controller could actually know (a keycard entry names the card's owner, so a stolen card frames them). The door Inspector shows it. Remaining: (1) an investigation affordance, so a crew member at a hatch or console can read its log into memory as evidence (a prompt line plus a `ReadAccessLog`-style `PhysicalInteractionRules` method); (2) a skill-gated wipe or falsify interaction that edits `RecordedName`/`RecordedId` while `ActorId` keeps the truth, discoverable like any other tamper (#12); (3) consoles, airlocks and machinery as well as hatches (pairs with #25 (4)); (4) Overseer reads the log over the control bus, so a partitioned network (#21) should hide it, as it does not yet.
