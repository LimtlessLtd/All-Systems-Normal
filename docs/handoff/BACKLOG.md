...
- **One hull breach can eject the whole crew within minutes (soak finding, 2026-09-24, not yet investigated).** Headless browser-mind soak, Secure Continuity, 12 crew, roster `SeededCrewRosterGenerator.Generate(15838, 12)`, `stationSeed: 202`, 1 robot. A fire in Overseer Isolation spreads to its access corridor (T+05:55) and breaches the hull (T+06:02). Between T+06:06 and T+06:08 all 12 crew die "ejected into space during decompression" in eight different rooms (Habitat Spine, Crew Quarters Access, Industrial Drop, Service Spur…). That suggests the breach decompresses much of the station through open doors, or the ejection check is too broad. The other 5 seeds had no deaths. Investigate the decompression spread/ejection rule against `ARCHITECTURE.md` physics expectations before tuning, together with the fire items above and #80.
