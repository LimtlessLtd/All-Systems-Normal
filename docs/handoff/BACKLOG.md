...
- **Long-run unattended survival (2026-09-24 soaks, #80).** Harness: a 40-seed browser-mind `StationSession` soak of the default station, 36h, no Overseer input, `ScenarioStatus` forced back to Running each minute. It is not committed; copy a `BrowserMindSession` from `DecompressionContainmentTests`. Three fixes so far:
  ...
  - **A hull breach can never be repaired** (open). Nothing clears `Room.HasHullBreach`, so a fire-driven breach vents the station for the rest of the run. A 20-seed × 36h soak (12 crew, `stationSeed = upkeepSeed = seed`), seed 15: an unfought Engineering fire (≥ 58 intensity, so fallback minds won't fight it) breaches at T+19:50 with 29 hatches open, and the whole station drains in ~6 minutes. After the O₂ fix, the crew live about 4h longer, then die of starvation, vacuum and O₂ in isolated pockets; 22 rooms are still in vacuum at T+36. Suggested slice: a physical `PatchHull` affordance (crew in the breached room, suited or after it is sealed, a timed task with engineering skill and storage materials) that clears the breach so the air loop can repressurise. Also worth checking with #80: containment only works for crew standing beside a hatch, and depth-scaled pressure loss drains deep rooms faster than anyone can react. Two other observations:
