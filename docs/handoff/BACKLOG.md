...
- Status: **in progress** — slice 1 shipped in #149: `Npc.CompletedTaskCounts` records successful recurring provisioning work by action (plant/tend, harvest, cook) only after authoritative completion; in-progress or failed work does not count. It is history only and changes no assignment or cognition.

  Remaining slices:
  1. Extend useful recurring job families such as maintenance without counting incidental actions.
  2. Surface repeated-work history to cognition so an NPC may decide a responsibility feels like "mine"; deterministic C# must not assign ownership.
  3. Add grounded irritation/resentment when another person visibly handles an established responsibility badly.
...
- **Fire response is still incomplete, but remote response is fixed (owner reports, 2026-09-23, `#new-ideas-and-functionality`).** The intended baseline remains that a normally competent crew should generally preserve itself and the station without requiring Overseer to babysit routine emergencies. Local fire-fighting parity was already shipped; the remote-responder slice now also ships: both deterministic fallback minds use `StationHazardSystem.FindRemoteFireForResponder` to nominate at most one capable, physically routed responder for a survivable fire in another compartment, after critical hunger/fatigue but before ordinary work. `ShouldFightFire` also rejects oxygen-starved, badly depressurised, smoke-heavy or critically injured cases.
