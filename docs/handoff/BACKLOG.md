...
- Status: **in progress** — slice 1 (private/sensitive memory flag) shipped: `Memory.IsPrivate` (default false) is new pure additive state — nothing in production sets it yet, matching this backlog's own established "foundation" pattern (e.g. idea #5's `NpcPlan`, idea #10's fixture-use telemetry). It already has real teeth rather than being fully inert: `ConversationTopicSystem.RecentNews` (the passive "pass on notable memories" pipeline `SYSTEMS.md` documents) now excludes any `IsPrivate` memory, so once something does get flagged private it can never leak involuntarily through ordinary small talk — only deliberate, trust-gated disclosure (the next slice) will be able to reveal it. Regression test confirms an otherwise-newsworthy private memory is withheld while an identical non-private one still passes on as before.

  Investigated whether an existing production memory-creation site should be flagged private in this same slice (e.g. `ActionResolver.NotifyPossessionWitnesses`, used for witnessing a possession hide/borrow/steal/destroy) and deliberately did not: that one function is reused uniformly for both benign (borrowing with consent) and illicit (theft) possession events, so flagging it wholesale would silently change already-shipped, heavily-tested possession/gossip behaviour as a side effect of an unrelated slice, and picking out only the illicit verbs is really slice 2's "what counts as a secret" design question, not slice 1's "does the flag work" question.

  Remaining slices: trust-gated disclosure decision (a deliberate `RevealSecret`-style affordance a holder chooses to use, and picking a first real producer of `IsPrivate` memories — witnessed wrongdoing is the natural candidate per the investigation above); a trust penalty for disclosing someone else's secret; blackmail as a pact-style interaction built on #1's `CrewPact`/`PactCoordinationSystem` plumbing.
...
- **Fire response is too narrow — likely why hull-breach deaths are so common (owner reports, 2026-09-23, `#new-ideas-and-functionality`: initial "Every run seems to end with a fire slowly breaching the hull..." plus 15:10 BST follow-up "Fires still seem to be ignored... nobody seems to care that they are all going to die").** The owner now states the intended baseline explicitly: absent player interference, a normally competent crew should generally preserve itself and the station rather than requiring Overseer to babysit routine emergencies. Investigated (confirmed by ChatGPT's independent triage in `#agentic-coordination`, 10:42 BST) and partially fixed — the fire-fighting-parity piece is done (see P1 above), but three deeper gaps remain, all in `BrowserMindSystem`/`RuleBasedAiDecisionService`:
  1. Both fallback minds only ever react to fire in the NPC's *own current room* (`HandleEmergencyReconsiderations`/`DecideAsync`'s dangerous-room branch keys off `CrewEnvironmentSafety.IsDangerous(currentRoom)`). Nobody travels toward a known fire elsewhere to help fight it, and nobody preemptively avoids a room they know is now dangerous until they happen to walk into it.
  2. `ActionKind.SealHazardRoom`/`VentHazardRoom` are never chosen by either fallback mind — they are reachable only through genuine Ollama LLM cognition (`OllamaAiDecisionService`/`NpcPromptBuilder`, whose STATION STATUS-PANEL block already exposes full station-wide fire/smoke readings and explicitly invites "sealing a fire, isolating smoke, or deliberately venting a compartment" — that part of the design already works, if Ollama is actually reachable). Since the deployed Pages build runs `BrowserMindSystem` exclusively (no server, no Ollama), this means doors are *never* sealed around a fire in that build at all — matching the owner's report precisely. Note when picking this up: `SealHazardRoom` requires the actor to be *inside* the room being sealed and closes every operable door around it in one action (`StationHazardSystem.TryExecuteCrewAction`) — a naive "seal the room I'm in as I leave" decision could trap the actor if that room only has one door, so this needs a real design pass (e.g. only seal once a safer room is confirmed reachable via a *different* still-open door), not a quick copy of the LLM's affordance into the fallback minds.
  3. No communication/alert mechanism exists for a fire specifically: nobody who discovers a fire tells anyone else about it (no `ReportConcern`-style broadcast, no "commander" concept issuing orders — chain-of-command/orders is itself still just an *owner idea*, not built). A witnessed fire could plausibly feed the existing gossip/News pipeline (`ConversationTopicSystem`) the same way other important events already do, once someone treats "I saw a fire in X" as a sufficiently important memory.
  
  Not attempted in the same run as the fire-fighting-parity fix: each of these is a real behavioural addition (not mechanical convergence) and deserves its own scoped slice with parity consideration between the two fallback minds, per the pattern P1's other slices already established.
