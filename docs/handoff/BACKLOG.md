...
- **Unattended stations still lose about a quarter of their crew within 36h (2026-09-24 soaks).** Harness: a 40-seed browser-mind `StationSession` soak of the default station, 36h, no Overseer input, `ScenarioStatus` forced back to Running each minute. Main lost 168 of 323 crew: 87 to oxygen deprivation, 33 to starvation and 31 to decompression, with only 11 of 40 seeds losing nobody. Two food-chain bugs are now fixed (see `SYSTEMS.md` → hydroponics): workers at a wall-side grow bay never counted as arrived, so bays were never planted; and a merely worn grow system halved growth. The same soak now loses 76 of 323 (38 oxygen deprivation, 25 decompression, 3 starvation), and 24 of 40 seeds lose nobody. What remains is mostly fire. Example, seed 5 (5 crew): an unfought fire spreads along the Primary Spine from T+27:30, breaches the hull at T+28:41 and burns the corridors' oxygen out while crew shuttle Storage↔Storage Access. Small crews also show ripe bays left unharvested and produce left uncooked for 8h+ (likely nobody with the horticulture/galley skill ≥ 25 on shift). The critical-need wake veto above is now fixed and is a plausible cause of the seed-5 fire being ignored; re-run the same long soak before claiming that example resolved. Remaining next step after that measurement: horticulture/galley coverage for 4–5 crew rosters (#80 competence distribution).
