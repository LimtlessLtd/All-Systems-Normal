...
- Status: **in progress** — slice 1 (uniform roster contract, both runtimes, robot seeding count) shipped: see `SYSTEMS.md` → crew rosters. "Keep crew alive" objective targets were already taken from the actual roster. Remaining: (2) size `PlannedCrewCount`/hydroponics and any other provisioning from the sampled crew instead of the scenario's fixed 12. It is currently oversupplied for small crews, which is safe. `StationGenerationConstraints` is a shared per-scenario instance with get-only collections, so this needs a copy-with-overrides helper, plus a test proving it copies every property. (3) Long-run viability coverage for representative combinations, together with #80. A quick headless browser-mind soak on 6 seeds × 24h showed no deaths for 4×1, 4×4 and 8×2. Also consider the upper bound: the owner's examples top out at 12 crew, so `MaxCrew` is 12 until they say otherwise.
...
- **One fire-driven hull breach still depressurises the whole station (soak finding, 2026-09-24).** Headless browser-mind soak, Secure Continuity, 12 crew, roster `SeededCrewRosterGenerator.Generate(15838, 12)`, `stationSeed: 202`, 1 robot. An unfought fire in Overseer Isolation breaches the hull at T+06:02. With 28 of 29 doors open, `FindVacuumDepths` reaches all 29 rooms, every room is at 0 kPa by about T+06:10, and all 12 crew die of decompression by T+06:20. (The other 5 seeds had no deaths.) The immediate defect is fixed: until then, anyone anywhere on the station was "ejected into space, no body aboard". Ejection is now limited to the exposed compartment and the room directly behind it (see `SYSTEMS.md`), which leaves bodies and roughly 10 extra minutes to react. The survival gap remains, and it is the fire open issue's gap 1 above: no fallback mind ever closes a hatch against a hazard, so nobody isolates the breach. Candidate designs, still needing the same design pass: (a) crew fleeing a depressurising room close the hatch behind them once they're through, a physical door action cognition chooses and no trap risk because they're already outside; or (b) a crew "seal the breach" choice on the far side of the hatch. Auto-closing pressure bulkheads would take door control from the Overseer and is a product call. Per-room pressure loss also falls only mildly with depth (`80 / (1 + 0.75·depth)` kPa/min), so even rooms 7 hatches away empty within ~5 minutes; revisit with #69 (slow hull leaks).
