...
- Status: **in progress**. Slice 1 is shipped (see `SYSTEMS.md` → seated eating): capacity-1 chair resolution for `Eat`, the seated/standing stress consequence, and the cognition `SEATS:` line. Eating still happens only in the galley (`IntentExecutionSystem` routes `Eat` to `kitchen`), which has four authored chairs. Remaining: (1) let eating happen in the other dining-capable rooms (recreation, quarters, medical bedside) so a full galley has somewhere else to go, with enough chairs and tables in each; (2) per-room chair/table materials and colours, browser-checked; (3) a seated pose for crew tokens at a chair, if the current token doesn't already read as seated.
