...
- Size: large (slices: pact data model + deterministic record/query/settlement API — #96; propose/accept affordance — shipped; resolution check (fulfil/break) exposed to cognition; gossip-able claim wiring)
- Status: ready — #96 supplies the deterministic pact foundation and honour/break relationship+memory consequences. `ProposePact`/`AcceptPact` are now real cognition affordances: `ProposePact` (LLM, co-located crew target, free-text promise in `Reason`) creates a `PactProposal` on the target (`PactCoordinationSystem`, mirrors `PendingShutdownTeamInvitation`'s 15-minute-expiry shape); `AcceptPact` turns a matching pending proposal into a real `CrewPactSystem.TryCreate` pact. Wired into `NpcPromptBuilder` (ACTIVE PACTS / PENDING PACT PROPOSAL sections + guidance) and `OllamaAiDecisionService.Validate`. Deliberately LLM-only (`BrowserMindSystem`/`RuleBasedAiDecisionService` don't choose it), matching the existing precedent for several social affordances (CoordinateWork, AssistCrew, ReassureCrew, RequestHelp, CheckOnCrew, Talk) that are never chosen by the deterministic fallback ladders. Pact `Kind` is always `Other` for LLM-proposed pacts for now (no free-text-to-enum inference); a future slice could let the LLM signal a structured kind if it becomes mechanically relevant. The next slice is exposing `TryFulfill`/`TryBreak` as a resolution decision to cognition (currently only reachable from tests) plus gossip-able claim wiring.
