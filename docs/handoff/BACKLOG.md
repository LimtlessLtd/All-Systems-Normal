...
### 1. Promises, deals and grudges (pacts)
...
- Size: large (slices: pact data model + deterministic record/query/settlement API — #96; propose/accept affordance — shipped; resolution check (fulfil/break) exposed to cognition — shipped; gossip-able claim wiring)
- Status: ready — #96 supplies the deterministic pact foundation and honour/break relationship+memory consequences. `ProposePact`/`AcceptPact` are real cognition affordances: `ProposePact` (LLM, co-located crew target, free-text promise in `Reason`) creates a `PactProposal` on the target (`PactCoordinationSystem`, mirrors `PendingShutdownTeamInvitation`'s 15-minute-expiry shape); `AcceptPact` turns a matching pending proposal into a real `CrewPactSystem.TryCreate` pact. `FulfillPact`/`BreakPact` now expose the resolution decision itself: the promisor (only — a promisee cannot settle someone else's promise) can choose either for any of their own active pacts (targeted by pact Id from `YOUR ACTIVE PACTS`), whenever it feels right to resolve, not only at a deadline; `ActionResolver` stages the choice, `PactCoordinationSystem.Tick` settles it via the existing `CrewPactSystem.TryFulfill`/`TryBreak` (which already writes the memory/trust/resentment consequences). All four are wired into `NpcPromptBuilder` (ACTIVE PACTS / PENDING PACT PROPOSAL sections + guidance) and `OllamaAiDecisionService.Validate`. Deliberately LLM-only (`BrowserMindSystem`/`RuleBasedAiDecisionService` don't choose any of them), matching the existing precedent for several social affordances (CoordinateWork, AssistCrew, ReassureCrew, RequestHelp, CheckOnCrew, Talk) that are never chosen by the deterministic fallback ladders. Pact `Kind` is always `Other` for LLM-proposed pacts for now (no free-text-to-enum inference); a future slice could let the LLM signal a structured kind if it becomes mechanically relevant. The only remaining slice is gossip-able pact claim wiring (surfacing a fulfilled/broken pact as something a witness or the parties themselves can retell, reusing the existing gossip/evidence pipeline).
...
- Size: large (slices: per-NPC belief store updated on sighting; retarget social/check-on goals onto the belief; expose ask/search affordances to cognition)
- Status: ready — owner called this "extremely important." Supersedes the former Open issues bullet on the same gap (`IntentExecutionSystem` comparing against `target.CurrentRoomId`), now merged here.
