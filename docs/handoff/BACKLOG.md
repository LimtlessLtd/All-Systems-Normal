...
- Idea: "NPCs shouldn't know where Sarah is. They know 'I last saw Sarah in Hydroponics three hours ago.' Searching then becomes physically meaningful: ask someone, check her workplace, inspect her quarters, follow blood, etc."
- Outcome: social/check-on/search intents target each NPC's own last-known-location belief (room + observed-at time), updated only through existing perception/sighting events, instead of the true `target.CurrentRoomId`; search behaviour (ask another NPC, check known haunts, follow evidence) becomes a real affordance the LLM can choose.
- Size: large (slices: per-NPC belief store updated on sighting — already exists, see below; retarget social/check-on goals onto the belief; expose ask/search affordances to cognition)
- Status: ready — owner called this "extremely important." Supersedes the former Open issues bullet on the same gap (`IntentExecutionSystem` comparing against `target.CurrentRoomId`), now merged here. Scoped (not yet started) this run: the belief store the first slice needs already exists and does not need building — `Npc.LastSeenCrew` (`Dictionary<Guid, CrewSighting>`, `CrewSighting` = id/name/room/`SeenAt`) is already populated on genuine line-of-sight by `PerceptionSystem.Tick` and is already the mechanism `MissingPersonSystem` uses for its own "unseen for N hours"/danger-evidence checks. The actual gap is narrower and more concentrated than it first looks: every social/check-on intent (`Talk`, `Socialize`, `Argue`, `RequestHelp`, `CheckOnCrew`, `AssistCrew`, `CoordinateWork`, `ReassureCrew`, `MisleadCrew`, `ReportConcern`, `ProposePact`, `RecruitShutdownAlly`) routes through one shared method, `IntentExecutionSystem.ExecuteSocialIntent`, which resolves both its "are we already together" fast path and its pathing destination from the target's true `target.CurrentRoomId` — one call site to change, not many. But it is a real behaviour change with open design questions that need a decision before touching it, which is exactly why last run and this run both declined to start it speculatively:
  1. **No sighting yet** (fresh scenario, or two crew who have simply never crossed paths): fall back to `CrewDutySchedule.ExpectedDutyRoomId(target.Role, state.Elapsed)` (the same public-schedule heuristic `MissingPersonSystem` already uses), not to `target.CurrentRoomId` — but `Prisoner`/roles without a duty schedule need a decision (idle-guess room? fail the intent with "I don't know where to find them"?).
  2. **Arrival at a stale belief**: once the actor reaches the last-known/believed room and the target genuinely is not there (the whole point of the idea), the intent needs to resolve to a clear failure (`FailIntent`, which already writes a memory + small stress bump) rather than silently re-pathing toward the target's live position — `ExecuteSocialIntent` does not currently have an "I arrived and they're not here" branch at all, since today it can't miss.
  3. **Blast radius**: this touches every social affordance the LLM has at once, not one at a time, and at least `IntentExecutionSystemTests.SocialIntent_WalksTowardTheTargetOneLegalSpaceAtATime` (and likely other social-intent tests in `CrewRoutingAndPresentationRegressionTests.cs`, `EmergentWorldSystemsTests.cs`, `InvestigationAndScenarioTests.cs`, `V010EInteractionTests.cs`) assert today's ground-truth-homing behaviour directly and will need deliberate rewriting (co-located `LastSeenCrew` setup, or an accepted duty-room fallback), not just a mechanical pass.
  A first slice should land only questions 1–2 answered plus the `ExecuteSocialIntent` retarget and its test rewrites; the ask/search-affordance slice (idea outcome's "ask someone, check her workplace, inspect her quarters, follow blood") is deliberately separate and should not block it.
...
- Size: large (slices: possession data model + ownership; borrow/steal/hide/return/destroy interactions; witness/search-based discovery; accusation/grudge/memory consequence wiring)
- Status: ready
