...
- Size: large (slices: per-NPC belief store updated on sighting — already exists; retarget social/check-on goals onto the belief — done, see below; expose ask/search affordances to cognition)
- Status: ready (first slice shipped). `IntentExecutionSystem.ExecuteSocialIntent` (every social/check-on intent: `Talk`, `Socialize`, `Argue`, `RequestHelp`, `CheckOnCrew`, `AssistCrew`, `CoordinateWork`, `ReassureCrew`, `MisleadCrew`, `ReportConcern`, `ProposePact`, `RecruitShutdownAlly`) now paths toward the actor's belief once not co-located — `npc.LastSeenCrew[target.Id].RoomId` when a sighting exists, else `CrewDutySchedule.ExpectedDutyRoomId(target.Role, elapsed)` — instead of `target.CurrentRoomId`. Physical co-location stays ground-truth (real perception, not omniscience). Every `CrewRole` including `Prisoner` already has a `WorkRoutes` entry, so the "roles without a duty schedule" question resolved itself: no special-casing needed.

  **Arrival-at-a-stale-belief decision, revised from the original scoping note**: the note proposed a hard `FailIntent` (memory + stress, intent cleared) when the actor reaches the believed room and the target genuinely isn't there. Implemented and tested that way first, then caught empirically — via `MedicalCareFlowRegressionTests.InjuredCrewMember_IsTreatedDuringAnOrdinaryShift(seed: 202)`, which started failing — that it causes real thrash: a doctor and a moving patient can repeatedly just-miss each other, and each miss fully abandons the pursuit with no re-acquire mechanism to recover it (nothing currently re-issues `AssistCrew` without a fresh direct sighting). Shipped the softer behaviour instead: arriving at a stale belief reports the miss (`Idle`, "X is not here, I last knew them to be around Y") but **keeps the intent alive**, mirroring the existing sealed-route pattern — `BelievedRoomId` is recomputed fresh every tick, so the actor automatically resumes pursuit the moment a newer sighting updates their belief, and otherwise the goal expires normally through the existing intent-lifetime timeout. No memory/stress cost for a mere miss. The harder "give up and need to actively search" semantics need the ask/search-affordance slice below to be safe (a real way to recover instead of dead-ending), so are deliberately deferred rather than a second attempt at a hard fail.

  Blast radius was smaller than feared: audited every social-intent test in the suite and only `IntentExecutionSystemTests.SocialIntent_WalksTowardTheTargetOneLegalSpaceAtATime` actually exercised the non-co-located path with a mismatched actor/target room and no prior sighting (updated to seed a matching sighting first); every other social-intent test either starts co-located already or drives `CurrentAction` directly, bypassing `ExecuteSocialIntent` entirely. 3 new regression tests (no-sighting duty-schedule fallback; miss-without-giving-up plus automatic recovery on a fresher sighting).

  Remaining slice: the ask/search-affordance layer itself (idea outcome's "ask someone, check her workplace, inspect her quarters, follow blood") — not started.
