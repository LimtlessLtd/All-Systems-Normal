...
- **Unattended stations lose about half their crew within 36h (found 2026-09-24, #92 soak).** A 40-seed browser-mind `StationSession` soak of the default station ran 36h with no Overseer input and `ScenarioStatus` forced back to Running each minute. Main lost 154 of 323 crew: 81 to oxygen deprivation, 33 to starvation, 26 to decompression, 7 ejected, plus a few fights and fires. Only 13 of 40 seeds lost nobody, and several lost everyone. The 8h soaks quoted elsewhere (0 deaths) never reach this. It is the owner's #80 target ("the space station should be able to survive by itself indefinitely"), and oxygen deprivation is the largest cause. Next step: root-cause the O₂ deaths on a few seeds (grid shedding Engineering on worn reactors? scrubber/O₂ generator wear with no maintenance?) before any competence tuning.
