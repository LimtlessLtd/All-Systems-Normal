...
- **Standalone assignments write into campaign history.** `CampaignProgressionSystem.CaptureCompletedMission` has no campaign-membership guard, so resolving Containment Transfer (a standalone assignment) adds a `MissionHistory` entry, replaces the campaign crew/station snapshot and can raise the reveal stage. That contradicts `SYSTEMS.md` → Standalone ("never touch campaign continuity"). The fix is a one-line `ScenarioCatalog.Campaign` membership check, but `ScenarioRosterPolicyTests.PrisonerFieldsSurviveCrewContinuityAndCampaignPersistenceRoundTrip` captures a Containment Transfer state on purpose to round-trip prisoner fields. Move that test onto a campaign scenario with prisoners added (or call `CaptureCrew` directly) in the same PR. Found during #103.
