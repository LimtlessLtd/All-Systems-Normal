...
- Status: in progress (slices 1-3 shipped, slices 4-5 remain). The two closest existing precedents to build on:
  ...
  - Slice order: (1) **done** — `FacilitySeeder.SeedPersonalPossessions` gives each crew member 1-2 items (deterministic on name, like `InitialBond`) at roster generation, held by the owner (`CurrentHolderId = OwnerId`, not hidden, not destroyed); the owner's own `KnownPossessionIds` is seeded too but nobody else's is. Covered by `PersonalPossessionSeedingTests.cs`. (2) **done** — `ActionKind.HideItem`/`ReturnItem`, LLM-only (like the pact affordances), let an owner hide their currently-held possession in their current room (`ActionResolver.TryHidePossession`, auto-attributing a Locker/Cabinet/Crate/StorageRack fixture in that room when one exists) or retrieve one they previously hid, but only while physically standing in the room where it is hidden (`ActionResolver.TryReturnPossession`); `CrewAffordanceSystem.TryNormalizeTarget` gates both by ownership/holder-state, `NpcPromptBuilder` exposes a YOUR PERSONAL POSSESSIONS block. Resolves instantly like `FulfillPact`/door ops — no travel/co-location system needed since the owner already knows exactly where their own item is. Covered by `PersonalPossessionInteractionTests.cs`. (3) **done** — `ActionKind.BorrowItem`/`StealItem`, LLM-only, let any crew member take a possession they already have in `KnownPossessionIds`: `BorrowItem` only from a co-located holder whose Trust/Affinity toward the borrower clears a threshold (`ActionResolver.TryBorrowPossession`); `StealItem` (`TryStealPossession`) from a co-located holder unconditionally (the holder is right there, so it always costs trust/resentment and leaves them a memory) or from a hiding spot the actor knows about in their current room (no confrontation memory — the owner isn't present to notice). New `PossessionAwarenessSystem` (mirrors `MedicalEvidenceSystem.NoticeBlood`) grants ambient `KnownPossessionIds` to anyone co-located with a holder who can `PerceptionSystem.CanMakeOut` them; `ActionResolver.NotifyPossessionWitnesses` grants it to anyone who witnesses a hide/borrow/steal act itself, identified-vs-unattributed exactly like `PactCoordinationSystem.NotifyWitnesses`. Also fixed a real bug found while wiring this in: `OllamaAiDecisionService.Validate` never called `TryNormalizeTarget` for possession-targeted actions at all (only room/crew/door targets), so `HideItem`/`ReturnItem` were silently unreachable through a real LLM decision since they shipped in slice 2 — only tests that constructed the intent/action directly ever exercised them. Covered by `PersonalPossessionInteractionTests.cs`, `PossessionAwarenessSystemTests.cs`, `AiDecisionServiceTests.cs`. Also fixed a second small bug found in the same pass: `NpcPromptBuilder`'s YOUR PERSONAL POSSESSIONS status text only ever handled "with you" or a hidden-location description, so once slice 3 could leave `CurrentHolderId` pointing at a third party, the owner's own block rendered a garbled "hidden in " (empty) line instead of naming who has it — fixed to show "with {holder}", which, per the same "an owner always knows their own possession's exact state" precedent slice 2 already established, is also how an owner passively notices a borrow or theft without needing to physically go check. **Post-merge fix (2026-09-23):** ChatGPT's audit of this slice found a real omniscience-contract regression: `KnownPossessionIds` was a flat permanent `HashSet`, but `NpcPromptBuilder` and `StealItem`'s hidden-stash targeting both read the possession's *live* global state for anything ever in that set, so a single long-past sighting let an observer see every later unwitnessed transfer/hide and steal from a hiding spot they never actually witnessed. Fixed by replacing it with `Npc.KnownPossessions: Dictionary<string, PossessionSighting>`, an observer-specific belief snapshot mirroring `LastSeenCrew`/`CrewSighting` (refreshed by ambient co-located perception of a held item, or by witnessing a hide/borrow/steal act — `TryHidePossession`/`TryReturnPossession` previously never called `NotifyPossessionWitnesses` at all, also fixed). The prompt now renders each observer's belief, not live state, and a hidden-stash steal requires the actor's own belief to place the item in their current room, not just a live coincidence.
  - (4) **scoped, not started** — owner noticing an expected item missing → concern. The naive version ("periodic scan of expected location", mirroring `MissingPersonSystem.NoticeMissedExpectations`) turns out not to fit: unlike a missing person, a possession has no independent duty schedule giving an "expected location" apart from the owner's own last action on it, and the YOUR PERSONAL POSSESSIONS block (now fixed, see above) already gives the owner fully live, always-current status for their own items every tick — so passive noticing of *where it is* already works for free. What's still missing is only the "surprised realization" moment: whenever a possession was taken while the owner was its co-located holder (`TryBorrowPossession`/`TryStealPossession`'s `holder is not null` branch), the owner already gets an immediate memory — the only real blind spot is `StealItem` taking it from a **hidden stash while the owner is absent**, where nobody currently tells the owner anything. A slice here needs: (a) some marker so the "I noticed it's gone" memory fires exactly once per event, not every tick forever the item stays elsewhere (a plain live-state comparison alone would spam); (b) a decision on whether it should escalate like `MissingPersonConcern` (stages, search behaviour) or stay a single memory, given the discovery channel is already fully passive/ambient rather than requiring an active search step. Small once decided; roughly pact-slice sized.
  - (5) `DestroyItem` and its relationship/memory consequences — not started, no open design question.
