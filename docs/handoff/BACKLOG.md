...
- [ ] Ladder convergence onto one utility scorer — remaining duplication is now concentrated in the two large if/else-if chains themselves (need/danger/investigation/social priority ordering in `RuleBasedAiDecisionService.DecideAsync` vs `BrowserMindSystem.Decide`); the countermeasure, airlock, reachability, skill-formula and threshold sub-pieces they call into are all shared. Folding the two priority chains onto one shared utility scorer is the final piece.
