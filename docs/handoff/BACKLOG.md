...
- Status: **in progress**. Shipped: the geometry bug, and an authoritative origin plus growing front, with spread seeded at the hatch (see `SYSTEMS.md` → fire). Both were browser-checked on live fires. A 20-seed × 36h soak was identical to main (0 deaths of 240, same fire-minutes). Remaining: (1) a fire is still one front per room, so a second ignition source in an already-burning room doesn't start a second patch; (2) fire-fighting doesn't yet depend on reaching the front (suppression works from anywhere in the room); (3) the front has no memory once intensity drops, so a fire being beaten back shrinks toward its origin rather than leaving scorched ground.
...
- Status: **in progress**. Slice 1 shipped: actions with no valid target for this person right now are left out of the capability list, together with their target contracts (~33.6k → ~30.4k characters). Slice 2 shipped: rooms whose readings are all ordinary show as `nominal` (defined once in the section header), and topology lines drop the atmosphere link implied by the hatch state and the default "you can traverse" (→ ~25.6k). See `SYSTEMS.md` → Ollama decision budget. Next: preamble tightening (~8.2k), then the capability descriptions (~7k), then lower `num_ctx` toward 8192 and tighten `NpcPromptBudgetTests`. Every room still appears in status and topology, because the minds use the whole layout for evacuation/sealing plans. Only filter rooms if the owner wants that. Bullet 1 of the same message (decision output cap 300 → 750) was pushed by the owner directly; the `num_ctx` stop-gap and its budget test shipped alongside.
...
- Ollama crew generation (`OllamaCrewGenerator`) asks for 1800 output tokens with no `num_ctx`, so it uses Ollama's default window. Thinking models spend part of the output on reasoning, and the owner's 2026-09-24 screenshot shows `InvalidOperationException: The model did not return a valid crew roster.` Likely the same truncation as NPC decisions: give it an explicit `num_ctx` and a larger output budget, and cover the request body like `OllamaProviderBoundaryTests` does.
