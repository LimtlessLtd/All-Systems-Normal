...
- Status: **in progress**. Slices 1–2 are shipped (see `SYSTEMS.md` → seated eating): capacity-1 seat resolution for `Eat`, the seated/standing stress consequence, the cognition `SEATS:`/`DINING:` lines, and carrying a meal from the galley to eat in a recreation room or crew quarters (a mind's choice; fallback minds do it only when the galley has no free seat). Remaining: (1) medical bedside eating (a patient in Medical is not a dining room yet); (2) more chairs and tables in the lounge/quarters (the lounge has two sofas and one reading chair, the quarters one desk chair plus any packed extras) and per-room chair/table materials and colours, browser-checked; (3) a seated pose for crew tokens at a seat, if the current token doesn't already read as seated. Note: the deterministic hunger routine (`CrewRoutineSystem.ChoosePlan`) still always heads for the galley, so routine eaters stand when it is full; only mind-chosen `Eat` intents carry meals elsewhere. The routine sets a room plan, not an intent, so it can't do the collect-then-carry step. Leave it unless soaks show much standing.
...
- Status: **in progress** — sleeping-bed overlap is addressed by #96's distinct `BedUseRules` assignment. A generic post-movement separation experiment was rejected because it perturbed the established decompression soak; remaining non-bed overlap should be reproduced and fixed at its actual shared destination instead of adding a global crowd-collision layer. Known edge from the #190 review (medical bedside dining): `BedUseRules.AssignedBed` ignores bedside eaters, so a sleeper assigned a medical bed that an eater already holds still walks to that bedside point. `DiningSeatRules.OccupantOf` only evicts the eater once the sleeper physically arrives, so the two briefly share a point. Fix direction: let bed assignment treat a seated bedside eater as occupying that bed, or have both rules share one medical-bed occupancy query.
...
- Status: **in progress** — simulation slice implemented on the current PR: `BedUseRules` assigns concurrent sleepers distinct beds and only the assigned bed grants restorative sleep; excess sleepers wait without recovery instead of sharing. Remaining: render the authoritative `FixtureUsePose.Lie`/assigned-bed state as an actual lying sleeper pose in the shared UI, with real-browser validation; relationship-based bed sharing waits for #35. Audit of #188 (Claude, 2026-09-24): a 20-seed × 36h 12-crew browser-mind soak, before vs after: no deaths either way; mean fatigue 27.9 → 28.8; time at fatigue ≥ 90 0.25% → 0.60%; 2.2% of sleep-minutes had no free bed. Beds are keyed by `Label`, so a room with duplicate bed labels would lose capacity. The default Quarters' six "Double Bunk" fixtures now hold one sleeper each; if a double bunk should sleep two, give it two bed points rather than relaxing capacity.


### 97. Local Ollama must be verifiably active
- Source: https://limitlessltds-fzn8994.slack.com/archives/C0C395V4TCP/p1790246739336259 (2026-09-24)
- Idea: "Ollama is STILL never called! This is literally the biggest part of this game and theres never any calls made to the LLM at all! FIX THIS IMMEDIATELY!"
- Outcome: the local ASP.NET runtime demonstrably reaches its injected model service and the real OllamaSharp HTTP boundary; every provider request is counted before awaiting it, startup reachability is probed, failures/cancellations are visible instead of silently disappearing into fallback, and /debug can distinguish process-wide provider attempts from per-station completed cognition traces.
- Size: small health fix (server/provider integration regressions + payload-free runtime diagnostics + debug status).
- Status: **in progress (#195, ChatGPT)** — real GameSession cadence and the OllamaSharp /api/chat boundary are both now covered by green end-to-end regressions; production diagnostics/failure visibility are being hardened in the same PR.
...
- **Long-run unattended survival (2026-09-24 soaks, #80).** Harness: a 40-seed browser-mind `StationSession` soak of the default station, 36h, no Overseer input, `ScenarioStatus` forced back to Running each minute. It is not committed; copy a `BrowserMindSession` from `DecompressionContainmentTests`. Three fixes so far:
  ...
  Deaths went 168 → 76 → 11 of 323, and seeds with no deaths 11 → 24 → 36 of 40. Mean crew stress is now ~11.5, down from 34.5. The remaining deaths (4 seeds) are 7 oxygen deprivation and 4 decompression, still to be traced. Two other observations:
