...
- **Fire response is still incomplete, but remote response is fixed (owner reports, 2026-09-23, `#new-ideas-and-functionality`).** The intended baseline remains that a normally competent crew should generally preserve itself and the station without requiring Overseer to babysit routine emergencies. Local fire-fighting parity was already shipped; the remote-responder slice now also ships: `StationHazardSystem.Tick` marks at most one available capable responder per remote fire for immediate mind reconsideration (without choosing their goal), and both deterministic fallback minds use `FindRemoteFireForResponder` when deciding so a reachable survivable fire can pre-empt ordinary work after critical hunger/fatigue. Committed `CrewTaskState` work stays protected. `ShouldFightFire` also rejects oxygen-starved, badly depressurised, smoke-heavy or critically injured cases.
  ...
  Owner reaffirmed this (2026-09-23 21:38 BST): crew still prioritise mundane routines (food, chores) over an active fire. `FindRemoteFireForResponder`'s pre-emption only fires after critical hunger/fatigue thresholds, so a fire that isn't yet lethal loses to ordinary need-driven work — the priority ordering itself, not just reachability, likely needs revisiting once gaps 1/2 above are closed.
