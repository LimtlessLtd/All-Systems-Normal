...
- Status: in progress (slices 1-4 shipped, slice 5 remains). The two closest existing precedents to build on:
  ...
  - (4) **done** — owner noticing an expected item missing → a one-time "surprised realization" memory, not a `MissingPersonConcern`-style staged escalation: the discovery channel is already fully passive/ambient (the YOUR PERSONAL POSSESSIONS block is always live for the owner's own items), so there is no active-search step to stage, unlike a missing person's 12-hour unseen threshold. `PersonalPossession.OwnerNoticedCurrentHolder` (default true — nothing to notice, they hold it themself) is set false only by `ActionResolver.TryStealPossession`'s hidden-stash branch, the one case where the owner is absent and nobody already tells them (every other transfer path already grants the owner a direct memory when they were physically present as the holder taken from). New `PossessionTheftNoticeSystem` scans for `CurrentHolderId != OwnerId && !OwnerNoticedCurrentHolder` each tick, gives the owner one memory (importance 0.5, so it flows through `ConversationTopicSystem`'s existing News-retelling path like every other gossip-able memory), and flips the flag so it fires exactly once. Covered by `PossessionTheftNoticeSystemTests.cs`.
