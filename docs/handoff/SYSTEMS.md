...
- Interpersonal commitments have deterministic durable state via `CrewPact` / `CrewPactSystem`: parties, promise text, creation/settlement time and active/fulfilled/broken status. Cognition decides whether to make or honour a promise; C# only validates/records the commitment and applies concrete trust/affinity or resentment/stress/memory consequences when it is settled. Cognition proposes/accepts pacts via the `ProposePact`/`AcceptPact` affordances: `PactCoordinationSystem` turns a co-located `ProposePact` into a `PactProposal` on the target (expires after 15 minutes unanswered, mirroring `ShutdownCoordinationSystem`'s invitation shape), and `AcceptPact` settles a matching proposal into a real `CrewPactSystem` pact. LLM-only for now, like several other social affordances. Exposing `TryFulfill`/`TryBreak` as a resolution decision to cognition and gossip-able pact claims remain backlog work.
