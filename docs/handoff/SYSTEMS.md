...
- Personal possessions (owner idea #3, slices 1-3 of 5): `state.Possessions: List<PersonalPossession>` gives each crew member 1-2 small personally meaningful items (food stash, photograph, medication, tool or keepsake) at roster generation, deterministic on name like `InitialBond` (`FacilitySeeder.SeedPersonalPossessions`). Every item starts `CurrentHolderId` = its owner, not hidden, not destroyed. Cognition may `HideItem`/`ReturnItem` on their own possession (owner-only) via `ActionResolver.TryHidePossession`/`TryReturnPossession`, auto-attributing a Locker/Cabinet/SuitLocker/ToolCabinet/Crate/StorageRack fixture when hiding; both resolve instantly since the owner already knows exactly where their own item is. `BorrowItem`/`StealItem` (slice 3) let any crew member take a possession they already know about — `BorrowItem` from a co-located holder willing to lend it (`ActionResolver.TryBorrowPossession` gates on the holder's Trust/Affinity toward the borrower); `StealItem` (`TryStealPossession`) from a co-located holder unconditionally (instant negative trust/resentment for the holder, who is right there) or from a hiding spot the actor knows about in their current room (no confrontation memory, since the owner isn't present). Discovery is never omniscient: `PossessionAwarenessSystem` (mirrors `MedicalEvidenceSystem.NoticeBlood`) grants observer-specific `Npc.KnownPossessionIds` ambiently to anyone co-located with the holder who can `PerceptionSystem.CanMakeOut` them; witnessing a hide/borrow/steal act itself (`ActionResolver.NotifyPossessionWitnesses`, same identified-vs-unattributed pattern as `PactCoordinationSystem.NotifyWitnesses`) also grants knowledge. All possession affordances are LLM-only for now, like the pact affordances; `NpcPromptBuilder` exposes YOUR PERSONAL POSSESSIONS (always live/current for your own items, including naming whoever else currently holds one) and OTHER PEOPLE'S POSSESSIONS YOU KNOW ABOUT blocks. Remaining slices: owner noticing an expected item missing → concern, and `DestroyItem`; see `BACKLOG.md` → Owner ideas → #3.
