...
- Personal possessions (owner idea #3, slices 1-2 of 5): `state.Possessions: List<PersonalPossession>` gives each crew member 1-2 small personally meaningful items (food stash, photograph, medication, tool or keepsake) at roster generation, deterministic on name like `InitialBond` (`FacilitySeeder.SeedPersonalPossessions`). Every item starts `CurrentHolderId` = its owner, not hidden, not destroyed; the owner's own `Npc.KnownPossessionIds` is seeded but no one else's is — discovery of another crew member's possession is deliberately unimplemented until a later slice. Cognition may `HideItem` (tucks a currently-held possession away in the owner's current room, auto-attributing a Locker/Cabinet/SuitLocker/ToolCabinet/Crate/StorageRack fixture there when one exists) or `ReturnItem` (retrieves a previously-hidden possession, but only while physically standing in the room where it is hidden) via `ActionResolver.TryHidePossession`/`TryReturnPossession`; both resolve instantly, like the door/pact affordances, since the owner already knows exactly where their own item is. LLM-only for now, like the pact affordances; `CrewAffordanceSystem.TryNormalizeTarget` gates targets by ownership/holder-state and `NpcPromptBuilder` exposes a YOUR PERSONAL POSSESSIONS block. No borrow/steal/discovery-by-others/destroy interaction exists yet; see `BACKLOG.md` → Owner ideas → #3 for the remaining slice order.
