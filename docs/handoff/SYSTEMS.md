...
- Social/check-on intents (`Talk`, `Socialize`, `Argue`, `RequestHelp`, `CheckOnCrew`, `AssistCrew`, `CoordinateWork`, `ReassureCrew`, `MisleadCrew`, `ReportConcern`, `ProposePact`, `RecruitShutdownAlly`) path toward each actor's own belief of the target's location — their last direct `LastSeenCrew` sighting, or `CrewDutySchedule.ExpectedDutyRoomId` when they have never crossed paths — never the target's true live room; being physically co-located is real perception and still resolves instantly. Arriving at a stale belief and not finding the target reports the miss (`Idle`) without abandoning the goal: the belief recomputes every tick, so a fresher sighting resumes pursuit automatically, and the goal otherwise expires through the normal intent-lifetime timeout. There is no search affordance yet (ask another crew member, check known haunts, follow evidence) — a stale belief just keeps trying, it cannot yet be actively resolved.
