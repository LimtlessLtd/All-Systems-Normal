...
2. **P1: converge the two fallback decision ladders** (`BrowserMindSystem` / `RuleBasedAiDecisionService`). Airlock rules (#88), reachability (#91), the repair-skill formula (#93), the need thresholds (#95) and the turret/robot countermeasure decisions are done; the only remaining slice is folding the two if/else-if priority chains themselves onto one shared utility scorer.
