...
2. **P1: converge the two fallback decision ladders** (`BrowserMindSystem` / `RuleBasedAiDecisionService`). Airlock rules (#88), reachability (#91), the repair-skill formula (#93) and the need thresholds (#95) are done; next slice is folding the two if/else-if chains onto one shared utility scorer.
