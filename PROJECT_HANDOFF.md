...
- Fixture collision is authoritative for mobile entities, and procedural packing uses that same collision predicate. Local navigation stays visually freeform but deterministic: physical furniture/machinery gets crew-sized separation, real door approaches are reserved, explicit standing interaction points reserve clear floor space at placement time, and local routing falls back to a deterministic occupancy-grid detour if the visibility graph dead-ends. Bulkhead-integrated decoration may remain non-blocking. The visibility-graph safety margin remains `0.35` because fixture clearance is enforced separately; fallbacks must never reselect the actor's current waypoint.
