...
2. **P1: converge the two fallback decision ladders** (`BrowserMindSystem` / `RuleBasedAiDecisionService`). Airlock rules (#88), reachability (#91), the repair-skill formula (#93), the need thresholds (#95), the turret/robot countermeasure decisions and the shutdown-coordination/missing-person helpers are done; see `BACKLOG.md` → P1 for why the remaining reachability-dependent helpers need a behaviour review rather than a mechanical move. The only remaining slice is folding the two if/else-if priority chains themselves onto one shared utility scorer.
